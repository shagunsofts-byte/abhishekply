      ...
      {isDrawerOpen && (
        <>
          <motion.div
            ...
            exit={{ opacity: 0 }}
            onClick={handleClose}
            className="fixed inset-0 z-[210] bg-stone-950/40 backdrop-blur-sm"
          />
          <motion.div
            initial={{ x: '100%' }}
            animate={{ x: 0 }}
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-stone-100">
              <h3 className="font-serif text-xl font-bold text-stone-900 flex items-center gap-2">
                <ShoppingBag className="w-5 h-5 text-brass-600" />
                Your Quote List
              </h3>
            ...
            {submitted ? (
              <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
                ...
                <h4 className="font-serif text-xl font-bold text-stone-900 mb-2">Quote request sent</h4>
                <p className="text-sm text-stone-500 font-inter mb-6 leading-relaxed">
              ...
              <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
                ...
                <h4 className="font-serif text-lg font-bold text-stone-900 mb-2">Your quote list is empty</h4>
                <p className="text-sm text-stone-500 font-inter mb-6">
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                  {items.map(({ product, qty }) => (
                    <div key={product.id} className="flex gap-3 pb-4 border-b border-stone-100 last:border-0">
                      <Link to={`/product/${product.slug}`} onClick={handleClose} className="w-16 h-16 rounded-xl overflow-hidden bg-stone-100 shrink-0">
                        <img src={product.thumbnail} alt={product.name} className="w-full h-full object-cover" />
                      </Link>
                      <div className="flex-1 min-w-0">
                        <span className="text-[10px] font-outfit uppercase tracking-widest text-brass-600">{product.brand}</span>
                        <Link to={`/product/${product.slug}`} onClick={handleClose}>
                          <h4 className="font-outfit font-medium text-sm text-stone-900 leading-snug truncate hover:text-brass-600 transition-colors">
                            {product.name}
                          </h4>
                        </Link>
                        <div className="flex items-center justify-between mt-2">
                          ...
                          <button
                            ...
                            className="text-stone-300 hover:text-rust-600 transition-colors"
                ...
                {/* Footer form */}
                <div className="px-6 py-5 border-t border-stone-100 space-y-3 bg-stone-50/60">
                  <p className="text-xs font-outfit text-stone-400 uppercase tracking-widest mb-1">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-stone-200 text-sm font-inter outline-none focus:border-brass-400 bg-white"
                  />
                  <input
                    type="tel"
                    value={phone}
